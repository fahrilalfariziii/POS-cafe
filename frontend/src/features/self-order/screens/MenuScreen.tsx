...
import { IconCart, IconMinus, IconPlus, IconSearch } from '../../../shared/components/icons'
...
export function MenuScreen({
  ...
  function qtyFor(id: string) {
    return cart.filter((c) => c.productId === id).reduce((s, c) => s + c.quantity, 0)
  }

  return (
    <div className="flex h-full flex-col bg-paper">
      <div className="relative shrink-0">
        <img alt="" className="absolute inset-0 h-full w-full object-cover" src={IMG.headerBg} />
        ...
        <div className="relative flex flex-col items-center gap-3 px-5 py-6">
          ...
          <div className="text-center">
            ...
            <p className="text-sm font-medium text-white">Halo, Selamat Datang!</p>
        ...
        <div className="flex flex-col gap-6 px-5 pb-40 pt-4">
            ...
            {cats.map((cat) => (
              <button
                ...
                className={`h-10 shrink-0 rounded-full px-6 text-sm font-semibold ${
                  category === cat ? 'bg-mint text-[#526853]' : 'border border-clay/30 bg-[#eee] text-soil'
                }`}
              >
                {cat}
              </button>
          ...
          <div className="grid grid-cols-2 gap-3">
            {filtered.map((item) => {
              ...
              const last = [...cart].reverse().find((c) => c.productId === item.id)
              return (
                <div
                  key={item.id}
                  className={`flex flex-col overflow-hidden rounded-[12px] bg-white shadow-[0_4px_15px_rgba(74,124,89,0.06)] ${!item.isAvailable ? 'opacity-60' : ''} ${qty > 0 ? 'ring-2 ring-mint' : ''}`}
                >
                  <div className="relative h-[150px] bg-[#e2e2e2]">
                    <img alt={item.name} src={item.imageUrl} className={`h-full w-full object-cover ${!item.isAvailable ? 'saturate-0' : ''}`} />
                    {item.badge && (
                    ...
                    {!item.isAvailable && (
                      <div className="absolute inset-0 flex items-center justify-center bg-paper/40">
                        <span className="rounded-[6px] bg-[#ba1a1a]/90 px-2 py-1 text-xs text-white">Habis</span>
                      </div>
                    )}
                  </div>
                  <div className="flex flex-1 flex-col justify-between p-3">
                    <div>
                      ...
                      <p className="mb-2 line-clamp-2 text-xs text-soil">{item.description}</p>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-sm font-semibold">{formatCompact(item.price)}</span>
                      {item.isAvailable &&
                        (qty === 0 ? (
                          <button
                            className="flex size-8 items-center justify-center rounded-full bg-[#2f3e2a]"
                            onClick={() => onSelectItem(item)}
                          >
                            <IconPlus />
                          </button>
                        ) : (
                          <div className="flex items-center rounded-full border border-clay/30 bg-[#eee] p-[3px]">
                            <button className="size-6" onClick={() => last && onUpdateQty(last.cartId, -1)}>
                              <IconMinus />
                            </button>
                            <span className="w-4 text-center text-xs">{qty}</span>
                            <button className="size-6" onClick={() => onSelectItem(item)}>
      ...
      {cartCount > 0 && (
        <div className="absolute bottom-20 left-0 right-0 z-10 px-5">
          <button
            onClick={onGoCart}
            className="flex h-[72px] w-full items-center justify-between rounded-[12px] bg-ink px-4 text-white shadow-xl"
          >
            <div className="flex items-center gap-3">
              ...
              <div className="text-left">
                ...
                <p className="font-display text-xl font-semibold">{formatRupiah(cartTotal)}</p>
              </div>
            </div>
      ...
      <nav className="absolute bottom-0 left-0 right-0 z-10 flex h-20 items-center justify-around border-t border-clay/20 bg-[#eee]">
        <span className="text-xs font-medium text-sage">Menu</span>
        <button onClick={onGoOrder} className="text-xs font-medium text-soil">
          Pesanan
        </button>
      </nav>
    </div>
